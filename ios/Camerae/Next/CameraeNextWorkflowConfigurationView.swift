...
struct CameraeNextCaptureConfiguration: Codable, Equatable, Hashable, Sendable {
    ...
    var astroPhotoStackCount: AstroPhotoStackCount
    ...
    static let repeatableDefault = Self(
        ...
        astroPhotoStackCount: .defaultValue
    )

    static let astroDefault = Self(
        module: .astrophotography,
        repeatableKind: .photo,
        durationMinutes: 30,
        videoDurationSeconds: 30,
        ...
        referenceOpacity: 0.5,
        intervalSeconds: 8,
        usesAutomaticAstroExposure: true,
        astroExposureSeconds: 8,
        astroCapturesPerFrame: 3,
        astroPhotoStackCount: .defaultValue
    )
    ...
    var estimatedFrameCount: Int {
        ...
        case .astrophotography:
            if repeatableKind == .photo {
                return astroPhotoStackCount.rawValue
            }
            if repeatableKind == .video {
                return max(1, Int(seconds) * videoSettings.fps)
            }
            return AstroCaptureTimingPolicy.estimatedCaptureCount(
                duration: seconds,
    ...
    func normalizedForCapture() -> Self {
        guard module == .astrophotography else { return self }
        var normalized = self
        normalized.intervalSeconds = AstroCaptureTimingPolicy.normalizedInterval(intervalSeconds)
        normalized.usesAutomaticAstroExposure = true
        return normalized
    }
}
...
struct CameraeNextWorkflowConfigurationPresentation: Equatable, Sendable {
    ...
    let showsAstroPhotoStacking: Bool
    ...
    init(configuration: CameraeNextCaptureConfiguration) {
        ...
        adjustmentTitles = isVideo
            ? ["EV"]
            : isAstroPhoto
            ? []
            : isAstro
            ? [CameraeL10n.interval, CameraeL10n.capturesPerFrame]
        ...
        isAstroExposureControlEnabled = false
...
struct CameraeNextWorkflowConfigurationView: View {
    ...
    init(
        ...
        if let savedProfile, savedProfile.module == project.module {
            ...
            initialConfiguration.sourceFormat = CameraeSettingsStore.shared.defaultSourceFormat(
                for: project.module
            )
        ...
        initialConfiguration = initialConfiguration.normalizedForCapture()
    ...
    private func openCamera() {
        var resolved = configuration.normalizedForCapture()
    ...
    private var modeBinding: Binding<CameraeNextCaptureModeOption> {
        Binding(
            get: {
                if isAstro, configuration.repeatableKind != .video {
    ...
    private func selectCaptureKind(_ kind: RepeatableCaptureKind) {
        ...
        if var profile = projectCaptureProfile {
            ...
            configuration = profile.configuration(for: kind).normalizedForCapture()
            ...
            VStack(spacing: 12) {
                ...
                if !isAstro && configuration.repeatableKind == .photo {
                    ...
                    CameraeNextSegmentedControl(
                        ...
                        selection: $configuration.astroPhotoStackCount,
                        theme: theme,
                        height: 34
                    )
                    ...
                    HStack(spacing: 6) {
                        ...
                        Text(configuration.astroPhotoStackCount.isRecommended
                             ? "10 fotos oferece o melhor equilíbrio para começar."
                             : configuration.astroPhotoStackCount.requiresStableDarkScene
                             ? "20–30 fotos exigem cena escura e estável; movimento ou muita luz pode reduzir a qualidade."
                             : "5 fotos processa mais rápido em cenas com mais luminosidade.")
                    }
                    ...
                    if !presentation.showsAstroPhotoStacking {
                        CameraeNextSliderRow(
                            ...
                            Slider(
                                ...
                                in: AstroCaptureTimingPolicy.intervalRange,
                                step: 1
                            )
                        }
                        CameraeNextSliderRow(
    ...
    private func synchronizeProjectState() {
        ...
        do {
            ...
            projectCaptureProfile = profile
            configuration = profile.selectedConfiguration.normalizedForCapture()
        ...
        do {
            ...
            await planning.evaluate(
                ...
                capabilityProfile: .init(
                    supportedSourceFormats: [.heic, .jpeg, .dng],
    ...
    private var capturePlan: CapturePlan {
        get throws {
            ...
            let plannedDuration: TimeInterval = if !isAstro && configuration.repeatableKind == .photo {
                ...
                max(
                    AstroCaptureTimingPolicy.normalizedInterval(configuration.intervalSeconds)
                        * Double(configuration.astroPhotoStackCount.rawValue),
                    1
                )
            } else if configuration.repeatableKind == .video {
    ...
    private var captureResolution: CaptureResolution {
        guard configuration.repeatableKind == .video else { return .fullSensor }
